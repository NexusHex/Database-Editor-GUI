...
def main():
    ...
    def add_data():
        ...
        def close_win():
            addWin.destroy()
            messagebox.showinfo('Data Added', 'The data has been added to the database. Please press the \'Update Database\' button to see the new values')
    ...
    def clear_data():
        ...
        if ask_del == 1:
            ...
            messagebox.showinfo('Returning to Main Program', 'All data has been deleted, press the \'Update Database\' button to see the cleared database')
        else:
            messagebox.showinfo('Returning to Main Program', 'No data deleted, press the \'Update Database\' button to see the cleared database')

    def close_app():
        mainWin.destroy()
    
    close_button_frame = tk.Frame(mainWin, width=100, height=30, highlightbackground='black', highlightthickness=1)
    close_button_frame.grid(row=0, column=0, padx=20, pady=20)
    ...
    title_frame = tk.Frame(mainWin, width=65, height=20, highlightbackground='black', highlightthickness=1)
    title_frame.grid(row=0, column=1, pady=20)
    editor_title = tk.Label(title_frame, text='Database Editor HeptaFunction', font=('Arial', 20), anchor='center')
    editor_title.grid(row=0, column=0, padx=(20, 40), pady=20)
    program_ver = tk.Label(title_frame, text='Version 1.1.1')
    program_ver.grid(row=0, column=1, padx=10, pady=10)
    ...
    update_db_button = tk.Button(data_frame, text='Update Database', command=update_textbox)
    update_db_button.grid(row=1, column=0, padx=20, pady=20)
    ...
    def del_vals():
        ...
        messagebox.showinfo('Data Deleted', 'Data has been deleted, please update the database with the \'Update Database\' button.')
    ...
    del_vals_frame.grid(row=1, column=1, padx=(80, 20), pady=(20, 200))
    ...
    clear_data_frame.grid(row=2, column=1, sticky='e', padx=20, pady=20)
